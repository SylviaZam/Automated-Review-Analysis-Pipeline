...
Public by design: aggregates only and no verbatim answers. Every question is
charted and every count shown, always beside its base; small bases are marked so
a reader knows a percentage rests on few answers. `--min-n` can hold small bases
back and `--mask-small-counts` can hide tiny counts, but neither is on by default.
"""
...
from .pipeline import SMALL_BASE, Analysis, public_count
...
def _card(title: str, question: str, body: str, base: int, reportable: bool, min_n: int) -> str:
    ...
    note = ""
    if reportable and base < SMALL_BASE:
        note = f' &middot; <span class="flag">small base, read the counts not the percentages</span>'
    return (f'<div class="card"><h2>{html.escape(title)}</h2><p class="q">{html.escape(question)}</p>{body}'
            f'<div class="base">Base: {public_count(base)} answers{note}</div></div>')
...
Privacy: identifying columns removed at ingest ({m['pii_columns_dropped']} dropped); no verbatim answers are shown. Percentages are always paired with the number of answers behind them.</footer>
</main></body></html>"""
