#!/usr/bin/env python3
"""
Analyze customer survey answers and export a wide-format Excel report with per-product pie charts.

Features
- Demo Mode (no OPENAI_API_KEY): offline sentiment via VADER + keyword categories.
- API Mode (OPENAI_API_KEY set): robust JSON output via Chat Completions (response_format='json_object').
- Caching: on-disk cache to avoid re-calling the API for repeated (industry, question, answer) triples.
- Cost control: truncate long answers and cap max_tokens.
- Output: one sheet per Product (wide columns: Qx_Answer/Qx_Sentiment/Qx_Category),
         one 'Summary' sheet, and one 'Charts - <Product>' sheet with a pie per question.

Default output file name: 'data analysis output.xlsx'
"""
...
def call_openai_analyze(
    ...
    """
    Ask OpenAI for JSON {sentiment, category}. Uses response_format='json_object' to ensure valid JSON.
...
def analyze_dataframe_wide(
    ...
    Uses a simple on-disk cache to avoid duplicate calls.
    """
    ...
    dirty = False
    flush_every = 200  # write cache to disk periodically
    calls_since_flush = 0

    def get_sent_cat(q: str, ans: str) -> Tuple[str, str]:
        nonlocal dirty, calls_since_flush
        k = cache_key(industry, q, ans)
        if k in cache_dict:
            return cache_dict[k]
        if client is None:
            sent, cat = demo_analyze_answer(ans)
        else:
            # cost control: truncate very long answers
            ans_for_api = ans[:max_chars]
            sent, cat = call_openai_analyze(industry, q, ans_for_api, client)
...
def write_excel_wide(wide: pd.DataFrame, out_path: str) -> None:
    """Write wide sheets per product + Summary + per-question pie charts using XlsxWriter."""
        ...
        if summary_all is not None and not summary_all.empty:
            ...
            for prod, prod_df in summary_all.groupby("Product"):
                ...
                for i, (_, row) in enumerate(prod_df.sort_values("Question").iterrows()):
                    ...
                    # Labels & counts vertically (ensures all Excels plot correctly)
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Survey analysis → wide Excel with per-question pie charts (Demo/API modes).")
    ap.add_argument("--input", required=True, help="Path to input CSV.")
    ap.add_argument("--industry", required=True, help="Industry context (e.g., 'Fashion').")
    ap.add_argument("--output", help="Output Excel path. Defaults to 'data analysis output.xlsx'")
    ap.add_argument("--cache", default=".analysis_cache.json", help="Path to JSON cache file (default: .analysis_cache.json)")
    ap.add_argument("--max-chars", type=int, default=600, help="Max characters of answer sent to API (cost control).")
    args = ap.parse_args()

    # Client selection (Demo vs API)
